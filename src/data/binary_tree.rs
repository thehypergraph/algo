...
impl Node<usize> {
    ...
    pub fn remove(&mut self, value: usize) -> bool {
        match self.data {
            Some(data) => {
                if value == data {
                    match self.left {
                        Some(ref mut left) => match left.right {
                            Some(ref mut right) => match right.max() {
                                Some(max) => {
                                    self.data = Some(max);
                                    right.remove(max);
                                }
                    ...
                    match self.right {
                        Some(ref mut right) => match right.left {
                            Some(ref mut left) => match left.min() {
                                Some(min) => {
                                    self.data = Some(min);
                                    left.remove(min);
                                }
    ...
    pub fn height(&self) -> usize {
        match self.left {
            Some(ref left) => match self.right {
                Some(ref right) => {
